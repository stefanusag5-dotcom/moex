...
MOEX_STOCKS = {
    ...
    "ENPG":  ("BBG00F6NK0J8", "Эн+ Груп",            "энергетика"),
...
# Таймфреймы: label -> (tinkoff_interval, минут)
TF_MAP = {
...
async def fetch_candles_tinkoff(figi: str, interval: str, limit: int) -> pd.DataFrame | None:
    ...
    delta_days = max(1, (limit * interval_minutes) // 1440 + 5)
    dt_from = datetime.utcnow() - timedelta(days=delta_days)
...
# ══════════════════════════════════════════════
# FIGI индекса МосБиржи (IMOEX) в Tinkoff API
IMOEX_FIGI = "BBG004730394"

async def fetch_imoex_regime() -> dict:
    """
    Определяет режим IMOEX на дневном TF.
    Кеш 30 минут — не нужно пересчитывать часто.

    Логика (аналог BTC macro из старого бота):
      BULL:    цена > EMA20 > EMA50, наклон EMA50 положительный
      BEAR:    цена < EMA20 < EMA50, наклон EMA50 отрицательный
      NEUTRAL: всё остальное

    Возвращает:
      regime:  'bull' | 'bear' | 'neutral'
      label:   строка для показа
      allowed: ['LONG'] | ['LONG', 'WATCH'] | ['WATCH']
      ema20, ema50, price, slope_10d
    """
    ...
    try:
        df = await fetch_candles_tinkoff(IMOEX_FIGI, "CANDLE_INTERVAL_DAY", 120)
        if df is None or len(df) < 50:
            raise ValueError("Мало данных IMOEX")
...
{{"event": "самое важное событие", "weight": итоговый вес от -10 до 10, "summary": "одно предложение"}}"""
    ...
    if tech_signal == "🟩 LONG":
        ...
        status_map = {k: "НЕТ СИГНАЛА" for k in ("CONFIRMED","WEAK","WATCH","BLOCKED","NO_SIGNAL")}
...
def compute_tech_score(df: pd.DataFrame, mode_cfg: dict,
    ...
    if ema200 > 0:
        if close > ema200:
            long_score += 10
            long_r.append("Выше EMA200 (долгосрочный тренд)")
        else:
            short_score += 10
    ...
    if macd_h > 0:
        long_score += 12
        long_r.append("MACD гистограмма положительная")
    elif macd_h < 0:
        short_score += 12
        short_r.append("MACD гистограмма отрицательная")
    ...
    if bb_low > 0 and close < bb_low * 1.01:
        long_score += 8
        long_r.append("Касание нижней полосы Боллинджера")
    if bb_up > 0 and close > bb_up * 0.99:
        short_score += 8
        short_r.append("Касание верхней полосы Боллинджера")

    # ── 8. Объём ──────────────────────────────────────────────────────────
    if vol_r > 1.5:
        if close > ema20:
            long_score += 8
            long_r.append(f"Объём x{vol_r:.1f} (бычий)")
        else:
            short_score += 8
            short_r.append(f"Объём x{vol_r:.1f} (медвежий)")
...
def format_analysis(result: dict) -> str:
    ...
    lines = [
        f"📊 <b>{ticker} — {name}</b> | {sector.upper()}",
    ...
    lines += [
        ...
        f"<code>{bars}</code>",
        f"Причины: {' + '.join(treasons[:3])}",
        f"{regime['label']}",
    ]
    if rsi_div:
        lines.append(rsi_div)
    if candle and candle != "Обычная свеча":
        lines.append(f"Свеча: {candle}")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    tickers_str = " ".join(f"<code>{t}</code>" for t in list(MOEX_STOCKS.keys())[:15]) + "..."
    text = (
        "🏛 <b>MOEX Signal Bot</b>\n"
        "Анализ акций 1-2 эшелона МосБиржи\n\n"
        "<b>Команды:</b>\n"
        "/analyze TICKER — полный анализ (техника + новости + ИИ)\n"
        "/scan — сканер сигналов по всем акциям\n"
        "/news TICKER — только новости с ИИ-оценкой\n"
        "/market — обзор рынка (IMOEX + новости)\n"
        "/mode — сменить режим (LOW/MID/HARD)\n"
        "/tf — сменить таймфрейм\n"
        "/watchlist — список всех отслеживаемых акций\n"
        "/trades — открытые позиции\n\n"
        f"<b>Доступные тикеры:</b>\n{tickers_str}\n\n"
        "<i>Данные: Tinkoff Invest API (только чтение)</i>"
    )
    await update.message.reply_text(text, parse_mode="HTML")

async def cmd_watchlist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lines = ["🗂 <b>Список инструментов</b>\n"]
    sector_map: dict[str, list] = {}
    for ticker, (figi, name, sector) in MOEX_STOCKS.items():
...
async def cmd_analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "error" not in result:
        kb.append([
            ...
            InlineKeyboardButton("📈 1d", callback_data=f"analyze_{ticker}_1d"),
            InlineKeyboardButton("📊 1h", callback_data=f"analyze_{ticker}_1h"),
        ])
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("mode_"):
        ...
        ticker = data.split("_")[1]
        trades = load_trades()
        # Берём последний анализ из кеша если есть
...
# ══════════════════════════════════════════════
SCANNER_CHAT_IDS: list[int] = []  # заполняется через /scan_start
...
async def run_scanner_broadcast(app):
    ...
    for s in signals[:6]:
        ...
        fs_e = {"CONFIRMED": "✅", "WEAK": "🟡", "WATCH": "👀", "BLOCKED": "🚫"}.get(fs, "⚪")
