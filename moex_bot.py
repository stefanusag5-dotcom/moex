...
def load_watchlist() -> list[str]:
    ...
    return ["SBER", "GAZP", "LKOH", "GMKN", "ROSN", "NVTK", "YNDX", "TATN", "CHMF", "MOEX"]
...
def add_to_watchlist(ticker: str) -> tuple[bool, str]:
    ...
    if len(wl) >= 30:
        return False, "⚠️ Максимум 30 инструментов. Сначала удали что-нибудь: /remove TICKER"
    wl.append(ticker)
    save_watchlist(wl)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "<b>🗂 Ватчлист:</b>\n"
        "/watchlist — мой список инструментов\n"
...
async def cmd_watchlist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    wl = load_watchlist()
    lines = [f"🗂 <b>Мой ватчлист</b>  ({len(wl)}/30)\n"]
    for t in wl:
