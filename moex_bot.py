...
FUTURES = {
    # ── Валюта ────────────────────────────────────────────────────────────
    "SiZ5":  ("FUTSI0925000", "Доллар/Рубль (Si)",    "валюта",   1.0,    1000),
    "EuZ5":  ("FUTEU0925000", "Евро/Рубль (Eu)",      "валюта",   1.0,    1000),
    # ── Индексы ───────────────────────────────────────────────────────────
    "RIZ5":  ("FUTRI0925000", "Индекс РТС (Ri)",      "индекс",   10.0,   1),
    "MXZ5":  ("FUTMX0925000", "Индекс MOEX (MIX)",    "индекс",   0.25,   1),
    # ── Товары ────────────────────────────────────────────────────────────
    "BRZ5":  ("FUTBR0925000", "Нефть Brent (BR)",     "товар",    0.01,   10),
    "GDZ5":  ("FUTGD0925000", "Золото (Gold)",         "товар",    0.1,    1),
    "NGZ5":  ("FUTNG0925000", "Природный газ (NG)",    "товар",    0.001,  1),
    "SRZ5":  ("FUTSR0925000", "Серебро (Silver)",      "товар",    0.01,   1),
    # ── Акционные фьючерсы ────────────────────────────────────────────────
    "SBZ5":  ("FUTSB0925000", "Сбербанк (SBER)",      "акция",    1.0,    100),
    "GZZ5":  ("FUTGZ0925000", "Газпром (GAZR)",        "акция",    1.0,    100),
    "LKZ5":  ("FUTLK0925000", "Лукойл (LKOH)",         "акция",    1.0,    10),
    "GKZ5":  ("FUTGK0925000", "Норникель (GMKN)",      "акция",    1.0,    10),
    "RNZ5":  ("FUTRN0925000", "Роснефть (ROSN)",       "акция",    1.0,    100),
    "NKZ5":  ("FUTNK0925000", "Новатэк (NVTK)",        "акция",    1.0,    10),
    "VBZ5":  ("FUTVB0925000", "ВТБ (VTBR)",            "акция",    0.5,    10000),
    "MNZ5":  ("FUTMN0925000", "МТС (MTSS)",            "акция",    1.0,    100),
    "AFZ5":  ("FUTAF0925000", "Аэрофлот (AFLT)",       "акция",    1.0,    1000),
}
...
def calculate_sl_tp_stocks(signal: str, price: float, atr: float,
    ...
    raw_risk = max(atr * 1.5, price * 0.008)
    if is_long:
...
def compute_tech_score(df: pd.DataFrame, mode_cfg: dict,
                       vp_nodes: dict = None, imoex_regime: dict = None) -> tuple[str, int, list]:
    row = df.iloc[-1]
    ...
    if vwap > 0:
        if close > vwap * 1.001:
            long_score += 20; long_r.append(f"Выше VWAP (+{vwap_dev:.2f}%)")
        elif close < vwap * 0.999:
            short_score += 20; short_r.append(f"Ниже VWAP ({vwap_dev:.2f}%)")

    if day_open > 0:
        dev_open = (close / day_open - 1) * 100
        if close > day_open * 1.001:
            long_score += 10; long_r.append(f"Выше открытия дня ({dev_open:+.2f}%)")
        elif close < day_open * 0.999:
            short_score += 10; short_r.append(f"Ниже открытия дня ({dev_open:+.2f}%)")

    if is_trend:
        if ema9 > 0 and close > ema9 > ema20:
            long_score += 20; long_r.append("Импульс EMA9 > EMA20")
        elif ema9 > 0 and close < ema9 < ema20:
            short_score += 20; short_r.append("Слабость EMA9 < EMA20")
        if macd_h > 0:
            long_score += 10; long_r.append("MACD > 0")
        elif macd_h < 0:
            short_score += 10; short_r.append("MACD < 0")
    else:
        if rsi < mode_cfg["rsi_oversold"]:
            long_score += 20; long_r.append(f"RSI в зоне перепроданности ({rsi:.0f})")
        elif rsi > mode_cfg["rsi_overbought"]:
            short_score += 20; short_r.append(f"RSI в зоне перекупки ({rsi:.0f})")
        if bb_low > 0 and close < bb_low * 1.002:
            long_score += 10; long_r.append("Нижняя граница Bollinger")
        elif bb_up > 0 and close > bb_up * 0.998:
            short_score += 10; short_r.append("Верхняя граница Bollinger")

    if vol_r > 1.5:
        if close > vwap:
            long_score += 15; long_r.append(f"Всплеск объема x{vol_r:.1f} вверх")
        else:
            short_score += 15; short_r.append(f"Всплеск объема x{vol_r:.1f} вниз")

    if "Бычье" in candle or "снизу" in candle:
        long_score += 10; long_r.append(candle)
    elif "Медвежье" in candle or "сверху" in candle:
        short_score += 10; short_r.append(candle)

    _cand = "🟩 LONG" if long_score >= short_score else "🟥 SHORT/ВЫХОД"
    if vp_nodes:
        vp_pts, vp_reasons = vp_score_adjustment(vp_nodes, close, _cand)
        if "LONG" in _cand:
            long_score += vp_pts; long_r.extend(vp_reasons)
        else:
            short_score += vp_pts; short_r.extend(vp_reasons)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "<b>📈 Фьючерсы FORTS:</b>\n"
        "/futures CODE — анализ фьючерса (SiZ5, BRZ5, RIZ5...)\n"
        "/scan_futures — сканер фьючерсов\n"
        "/futures_list — все доступные\n"
        "/fadd CODE — добавить в ватчлист\n\n"
        "<b>🚀 Памп/Дамп детектор:</b>\n"
        "/pd — найти резкие движения прямо сейчас\n"
...
async def analyze_futures(code: str, tf: str = "15m", mode_cfg: dict = None) -> dict:
    ...
    if df is None or len(df) < 20:
        return {"error": f"Недостаточно данных для {code} [{tf}]. Попробуй /update_figi"}
...
async def cmd_futures_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("\n".join.join(lines), parse_mode="HTML")
...
async def cmd_scan_futures(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(
        f"📊 {s['ticker']}", callback_data=f"fut_{s['ticker']}_{tf}"
    ) for s in top]] if top else []
...
PUMP_DUMP_CONFIG = {
    "15m": {"threshold_pct": 3.5, "lookback": 3, "vol_mult": 2.0,
            "sl_pct": 1.0, "tp1_pct": 2.0, "tp2_pct": 3.5, "tp3_pct": 5.5},
    "5m":  {"threshold_pct": 2.5, "lookback": 4, "vol_mult": 2.5,
            "sl_pct": 0.7, "tp1_pct": 1.5, "tp2_pct": 2.5, "tp3_pct": 4.0},
    "1h":  {"threshold_pct": 5.0, "lookback": 2, "vol_mult": 1.8,
            "sl_pct": 1.5, "tp1_pct": 3.0, "tp2_pct": 5.0, "tp3_pct": 7.0},
}
...
async def cmd_trades(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = []
    if open_t:
        lines.append(f"📂 <b>Открытые ({len(open_t)}):</b>")
        for k, t in open_t.items():
            lines.append(format_trade_status(t))
            lines.append(f"  <code>/close_trade {t['ticker']}</code>")
            lines.append("")
...
def _format_scan_row(s: dict) -> str:
    ...
    score = s["tech_score"]; sector = s["sector"]; regime = s["regime"]["label"]
...
async def post_init(app):
    # Подгружаем FIGI из файла при старте (если уже запускали /update_figi)
    loaded = _load_figi_from_file()
    if loaded:
        logger.info(f"Startup: loaded {loaded} FIGIs from file")
    else:
        logger.warning("Startup: figi_result.json not found. Run /update_figi to fix FIGI values.")
