...
def _format_scan_row(s: dict) -> str:
    """Форматирует одну строку сканера: тикер, название, цена, причина."""
    ticker = s["ticker"]
    name   = s["name"]
    price  = s["price"]
    score  = s["tech_score"]
    ...
    # Берём только первую причину — самую важную
    ...
    event_str = f"  📎 {esc(event)}" if event and event != "нет значимых событий" else ""
    return (
        f"<b>{esc(ticker)}</b> — {esc(name)} <i>({esc(sector)})</i>\n"
        f"  💰 {price:,.2f}₽  Скор: {score}/100  {esc(regime)}\n"
        f"  📊 {esc(reason)}\n"
        f"  {fs_e} Новости: {fs} [{ew_s}]{event_str}"
    )
...
async def _run_scan(tickers: list[str], tf: str, mode_cfg: dict,
    ...
    """
    Сканирует список тикеров.
    Возвращает (long_sigs, short_sigs, watch_sigs) — только реальные сигналы.
    watch_sigs = скор >= 55 но нет сигнала (накопление).
    progress_cb — опциональный колбек для обновления прогресса.
    """
    ...
    for i in range(0, total, 5):
        ...
        for r in results:
            ...
            if sig == "🟩 LONG":
                ...
                short_sigs.append(r)
            elif r["tech_score"] >= 55:
                # На радаре — НЕ показываем в авторассылке,
                # только в ручном /scan чтобы не спамить
                watch_sigs.append(r)
        await asyncio.sleep(0.5)
        if progress_cb and i % 20 == 0 and i > 0:
            await progress_cb(i, total)

    long_sigs.sort(key=lambda x: -x["tech_score"])
    short_sigs.sort(key=lambda x: -x["tech_score"])
    watch_sigs.sort(key=lambda x: -x["tech_score"])
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not wl:
        await update.message.reply_text(
            "📭 Ватчлист пуст.\n/add TICKER — добавить\n/add_all — добавить все 68 инструментов")
        return

    msg = await update.message.reply_text(
        f"🔍 Сканирую {len(wl)} инструментов [{tf}]...\n"
        f"⏳ ~{max(1, len(wl)//10)} мин.", parse_mode="HTML")

    async def progress(done, total):
        try:
            await msg.edit_text(
                f"🔍 Сканирую... {done}/{total} [{tf}]", parse_mode="HTML")
        except Exception:
            pass
    ...
    total_signals = len(long_sigs) + len(short_sigs)

    if not long_sigs and not short_sigs and not watch_sigs:
        await msg.edit_text(
            f"😶 <b>Сигналов нет</b>\n"
            f"<i>{ts} | Просканировано: {len(wl)} инструментов</i>\n\n"
            f"Рынок в ожидании. Попробуй переключить режим на HARD: /mode",
            parse_mode="HTML")
        return

    lines = [
        f"🔍 <b>СКАНЕР MOEX</b> | {esc(mode_cfg['label'])} | {tf}",
        f"<i>{ts} | Просканировано: {len(wl)} | Сигналов: {total_signals}</i>",
        "",
    ]

    if long_sigs:
        lines.append(f"🟩 <b>ПОКУПКА ({len(long_sigs)}):</b>")
        for s in long_sigs[:8]:
            lines.append(_format_scan_row(s))
            lines.append("")

    if short_sigs:
        lines.append(f"🟥 <b>ВЫХОД/ШОРТ ({len(short_sigs)}):</b>")
        for s in short_sigs[:5]:
            lines.append(_format_scan_row(s))
            lines.append("")

    if watch_sigs:
        lines.append(f"👀 <b>На радаре — скор 55+ ({len(watch_sigs)}):</b>")
        lines.append("<i>Нет сигнала, но набирается сила</i>")
        for s in watch_sigs[:5]:
            ...
            reason = s["tech_reasons"][0] if s["tech_reasons"] else "—"
            lines.append(
                f"  <b>{esc(ticker)}</b> {esc(name)}  {price:,.2f}₽  "
                f"Скор:{score}  {esc(reason)}"
            )

    # Кнопки быстрого доступа к топ-3 сигналам
    kb = []
    top = (long_sigs + short_sigs)[:3]
    if top:
        kb.append([
            InlineKeyboardButton(f"📊 {s['ticker']}", callback_data=f"analyze_{s['ticker']}_{tf}")
            for s in top
        ])
    markup = InlineKeyboardMarkup(kb) if kb else None
...
# Хранит последние отправленные сигналы чтобы не дублировать
_last_broadcast_signals: set = set()

async def scanner_loop(app):
    """
    Фоновый сканер для авторассылки.
    Интрадей режим: каждые 30 минут в торговое время MOEX.
    Отправляет только НОВЫЕ сигналы (не дублирует предыдущие).
    """
    global _last_broadcast_signals
    while True:
        now_utc = datetime.now(timezone.utc)
        now_msk_hour = (now_utc.hour + 3) % 24
        now_msk_min  = now_utc.minute
        is_trading_day  = now_utc.weekday() < 5
        # MOEX: 10:00-18:50 МСК
        is_trading_time = (10 <= now_msk_hour < 18) or (now_msk_hour == 18 and now_msk_min < 50)
        ...
        if is_trading_day and is_trading_time and SCANNER_CHAT_IDS:
            ...
            # Вне торгов — сбрасываем кеш сигналов (новый день = новые сигналы)
            if not is_trading_time:
                _last_broadcast_signals = set()

        # Интрадей: каждые 30 минут
        await asyncio.sleep(1800)


async def run_scanner_broadcast(app):
    """
    Авторассылка: только реальные сигналы LONG/SHORT.
    НЕ показывает "на радаре" — это только для ручного /scan.
    Отправляет только новые сигналы (дедупликация по тикеру+сигналу).
    """
    global _last_broadcast_signals
    tf       = "15m"
    mode_cfg = TRADE_MODES["mid"]
    ...
    lines.append(f"<i>Детали: напиши тикер или /analyze TICKER</i>")
    text = "\n".join(lines)
